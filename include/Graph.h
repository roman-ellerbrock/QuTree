...
namespace qutree
{
     ...
     *
     * The graph class encodes the topology of a tensor network. Nodes, edges and leaves
     * are stored in maps. They can be given names which are stored in the maps as well.
     */
    ...
    public:
        ...
        Graph(
            const std::map<Node, Attribute> &&nodes,
        ...
        std::vector<Edge> inEdges(Node node) const
        {
            ...
            std::copy_if(edges_.begin(), edges_.end(), std::inserter(filtered_edges, filtered_edges.end()),
        ...
        /**
         * \brief Generate vector of edges sorted so that it can be sweeped over in TNS simulations
         **/
