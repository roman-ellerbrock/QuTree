...
/*	TEST (OperatorTree) {
		...
		for (const Node& node : optree) {
			if (node.isBottomlayer()) {
					ASSERT_EQ(2, node.shape().order());
					ASSERT_EQ(4, node.shape().lastDimension());
					ASSERT_EQ(4, node.shape().lastBefore());
			}
		}
	}*/

TEST (TensorTreeOperator, TTNO) {
    Tree tree = TreeFactory::balancedTree(12, 2, 2);
    Tree optree = TreeFactory::operatorTree(tree);
    ...
    for (const Node& node : optree) {
        ...
        for (size_t i = 0; i < node.shape().totalDimension(); ++i) {
            double r = abs(B[i]);
            ASSERT_EQ((r > 1e-15), true);
        }
    }
}

TEST (TensorTreeOperator, TTNOrep) {
    SOPd S;
    ...
    /// Check error > 0, then contract, then check that error < tteps
    double err0 = error(A, S, optree);
    ASSERT_EQ(1, (err0 > 1e-1));
    contractSOP(A, S, 2, optree, nullptr);
    double err = error(A, S, optree);
    ASSERT_EQ(1, (err < 1e-12));
}

TEST (TensorTreeOperator, TTNOrep_nonhermitian) {
    SOPd S;
    ...
    ASSERT_NEAR(0., rep[optree.topNode()][0][0], tteps);

    TTOcontractiond con(tree, optree);
    con.calculate(Psi, A, rep, Psi, optree);
}

TEST (TensorTreeOperator, Circuit_contraction) {
    mt19937 gen(time(NULL));
    ...
    ASSERT_NEAR(0., real(E(0, 0)), 1e-10);
}

TEST(TensorTreeOperator, contractCircuit_CNotChain) {
    mt19937 gen(time(NULL));
