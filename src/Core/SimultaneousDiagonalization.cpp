...
void SimultaneousDiagonalization::JacobiRotations(vector<FactorMatrixcd>& A,
	FactorMatrixcd& trafo) {
...
void SimultaneousDiagonalization::InitialTransformation(vector<FactorMatrixcd>& A,
	FactorMatrixcd& trafo) {
	FactorMatrixcd& B = A[0];
	Vectord ev(B.Dim1());
	B.cDiag(trafo, ev);
